...
function QuizCard({ item }) {
  ...
  return (
    <li className="facade__card card">
      ...
      {answered && (
        <div
          className={`facade__feedback ${correct ? "is-correct" : "is-wrong"}`}
          role="status"
        >
...
export default function Facade() {
  return (
    <SectionShell
      ...
      n={s.n}
      label={s.label}
      className="facade-section"
    >
      <h2 data-reveal>Kiedy partycypacja staje się fasadą?</h2>
      <p className="lead section__intro" data-reveal>
        Czasem procedura jest dochowana, a sensu brak. Oceń scenariusze: to
        jeszcze partycypacja, czy już fasada?
      </p>
