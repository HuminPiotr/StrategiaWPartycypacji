...
const KEY = { residents: 'residents', experts: 'experts', politics: 'politics' }

export default function GoldenRatio() {
  const [vals, setVals] = useState({ residents: 34, experts: 33, politics: 33 })
  ...
  const setOne = (id, v) => setVals((prev) => ({ ...prev, [id]: Number(v) }))

  return (
    <SectionShell id="ratio" n={s.n} label={s.label} className="ratio-section">
      <h2 data-reveal>Złota proporcja nie istnieje</h2>
      <p className="lead section__intro" data-reveal>
        Trzy składniki każdej strategii. Ustaw proporcje i zobacz, co ryzykujesz — bo nie ma
      ...
      <div className="ratio__panel" data-reveal>
        ...
        <div className="ratio__sliders">
          {ratio.components.map((c) => (
            <div key={c.id} className="ratio__slider-row" style={{ '--r-accent': `var(--${c.accent})` }}>
              <label className="ratio__slider-label" htmlFor={`r-${c.id}`}>
                <span className="ratio__dot" style={{ background: `var(--${c.accent})` }} aria-hidden="true" />
                {c.label}
        ...
        <p className={`ratio__comment ratio__comment--${comment.tone}`} role="status" aria-live="polite">
          {comment.text}
        </p>
      </div>
